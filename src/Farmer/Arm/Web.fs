                       ...
                       siteConfig =
                        {| alwaysOn = this.AlwaysOn
                           ...
                           cors =
                            ...
                            |> Option.toObj
                           vnetName = this.VNetConnectionName |> Option.defaultValue null
                        |}
                    |}
            |} :> _
...
type VirtualNetworkConnection = 
    { Site: LinkedResource
      ...
      IsSwiftNetwork: bool }
    static member createFrom location siteId subnetRef =
        { Site =  Managed siteId
          Location = location
          Kind = None
          CertBlob = None
    ...
    member this.SubnetConnectionName = 
        ...
        |> String.concat "_" 
    member this.ResourceId = virtualNetworkConnections.resourceId( this.Site.Name / this.SubnetConnectionName)
    interface IArmResource with
                ...
                properties = 
                    {| vnetResourceId = this.Subnet.ResourceId.Eval()
                       ...
                       dnsServers = 
                            ...
                            | [] -> null 
                            | ips -> List.map string ips |> String.concat ","
                       isSwift = this.IsSwiftNetwork |}
            |} :> _